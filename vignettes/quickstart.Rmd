...
Some of this difference is because of the size of each country's population. To compare cases on a per capita basis, divide cases by  population:

```{r fig.width=6}
some_countries <- all_countries %>%
  filter(country %in% c('Italy', 'United Kingdom', 'Spain', 'United States')) %>%
  mutate(cases_per_capita = cases_total/population_2019)

ggplot(some_countries, mapping = aes(x = date, y = cases_per_capita, colour = country)) +
  geom_line() +
  labs(x = NULL, y = "All reported Covid-19 cases per capita", colour = "Country") +
  theme_classic() +
...
To do this, we use `get_regional_data()`. We'll get the state-level data for the United States over time:
...
usa_states %>% filter(state %in% c('New York', 'Texas', 'Florida')) %>%
  ggplot(aes(x = date, y = cases_total, colour = state)) + 
  geom_line() +
...
Most countries have a hierarchy of political subdivisions. Where data are available, use the `include_level_2_regions = TRUE` argument to `get_regional_data`. We can dig down further into each US state by looking at data by county:
...
The same method will work for different countries where data are available, returning relevant sub-national units for each country. In Germany, data are available for Bundesland (federated state) and Kreise (district) areas.
...
ggplot(german_region_totals, aes(x = reorder(bundesland, -deaths_total), y = deaths_total)) +
  geom_bar(stat = 'identity') +
  labs(x = "Bundesland", y = "All reported Covid-19 deaths") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90),
        axis.title.y = element_text(hjust = 1),
        legend.position = "bottom") 
...
We can map Covid-19 data at national or subnational level, using any standard shapefile. In R, the `rnaturalearth` package already provides shapefiles for all country borders, and the largest (administrative level 1) subnational units for most countries. 

We provide the relevant national and subnational ISO codes, or local alternatives, to enable mapping. 

We can map worldwide country shapefiles using `rnaturalearth::ne_countries()`, and join Covid-19 data to the map using the `iso_code`.

In this example we use the `rnaturalearth` package to get a map of Indian states, and join Covid-19 data to these maps using the `iso_3166_2` column in both dataframes. We can plot using the `sf` package.

```{r fig.width=6}
# Load rnaturalearth and sf packages
library(rnaturalearth)
library(sf)

# Get map of Indian states
india_map <- ne_states(country = "India", returnclass = "sf")

# Get India state-level aggregate Covid-19 data
india_region_totals <- get_regional_data(country = "India", totals = TRUE)

# Join data to map
india_map_with_data <- left_join(india_map, india_region_totals, by = "iso_3166_2")

# Plot map using sf
ggplot(india_map_with_data) +
  geom_sf(aes(fill = deaths_total)) +
  labs(fill = "All reported Covid-19 deaths by state") +
  scale_fill_viridis_b(direction = -1, option = 5) +
  theme_minimal()
