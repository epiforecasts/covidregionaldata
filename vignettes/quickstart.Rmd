...
```{r, eval = FALSE}
usa_counties <- get_regional_data(country = "USA", level = "2")
```
...
```{r fig.width = 6}
get_regional_data(country = "Germany", totals = TRUE) %>%
  ggplot() +
  aes(x = reorder(bundesland, -deaths_total), y = deaths_total) +
  geom_bar(stat = "identity") +
...
map_data <- get_national_data(totals = TRUE) %>%
