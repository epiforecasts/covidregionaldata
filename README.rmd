...
```{r global_options, echo=F, message=F}
...
```{r getalldata, message=F, results='hide'}
allcountries <- covidregionaldata::get_national_data()
```
```{r showalldata, message=F, echo=F}
allcountries <- allcountries %>% arrange(desc(date))
kable(head(allcountries), caption = "All country Covid-19 data")
```
...
To get time-series data for national regions of a specific country, for example by region in the UK, use `get_regional_data()`:

```{r getukregions, message=F, results='hide'}
ukdata <- covidregionaldata::get_regional_data(country = "UK")
```
```{r showukregions, message=F, echo=F}
ukdata <- ukdata %>% arrange(desc(date))
kable(head(ukdata), caption = "Uk regional Covid-19 data")
```

Now we have the data we can create plots, for example the evolution of the number of cases for each region:

```{r plotukregions, warning=F, message=F}
ukdata %>%
...
To get time-series data for subnational regions of a specific country, for example by local authority in the UK, use `include_level_2_regions = TRUE`:

```{r getuksubregions, message=F, results='hide'}
ukdata_sub <- covidregionaldata::get_regional_data(country = "UK", include_level_2_regions = T)
```
```{r showuksubregions, message=F, echo=F}
ukdata_sub <- ukdata_sub %>% arrange(desc(date))
kable(head(ukdata_sub), caption = "Uk sub-national regional Covid-19 data")
```

With the sub-national data we can evaluate the evolution of cases for local authorities within each region, for example the East Midlands:

```{r plotuksubregions, warning=F, message=F}
ukdata_sub %>%
...
For further demonstrations see [Quick start vignette](https://github.com/epiforecasts/covidregionaldata/blob/master/vignettes/quickstart.Rmd#L84)
