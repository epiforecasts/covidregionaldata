...
get_southafrica_regional_cases_only_level_1 <- function() {
  ...
  # Set region names
  region_names <- tibble::tribble(~ level_1_region_code, ~ region_level_1,
                                  'ZA-EC', 'Eastern Cape',
                                  'ZA-FS', 'Free State',
                                  'ZA-GP', 'Gauteng',
  ...
  data <- dplyr::bind_rows(cases, deaths, .id = "data") %>%
    dplyr::mutate(data = factor(data, c(1,2), c("cases_total", "deaths_total")),
    ...
    dplyr::mutate(level_1_region_code = paste0("ZA-", level_1_region_code),
