...
#'
#' @param country Character A string specifying the country to get data from.
#'  Not case or language dependent. Defaults to all countries.
#' @param totals Boolean. If TRUE, returns totalled data per country up to
#'  today's date. If FALSE, returns the full dataset stratified by date and
#'  country.
#' @param source Character A string specifying the data source: "WHO", or
...
get_national_data <- function(country = NULL, totals = FALSE, source = "who") {
  stopifnot(is.logical(totals))
  stopifnot(is.character(source))

  source <- tolower(source)
  source <- match.arg(source, choices = c("ecdc", "who"))
  ...
  if (missing(country)) {
    ...
    if (is.na(input_country_name)) {
      stop(
        "Country name not recognised. Please enter a character string,
  ...
  if (totals) {
    data <- data %>%
      ...
      totalise_data() %>%
      dplyr::select(-.data$recovered_total, -.data$hosp_total,
                    -.data$tested_total) %>%
      dplyr::arrange(-.data$cases_total) %>%
      dplyr::rename("country" = "region_level_1",
                    "iso_code" = "level_1_region_code")
  ...
  if (source == "who") {
    ...
    data <- data %>%
      dplyr::select(
        date, un_region, who_region, country, iso_code,
        cases_new, cases_total,
        deaths_new, deaths_total,
        recovered_new, recovered_total,
        hosp_new, hosp_total,
