...
D <- R6::R6Class("D",
  ...
  public = list(
    common_data_urls = list(
      main = "tests/testthat/custom_data/ecdc.csv"
    ),
...
test_that("DataClass can return data", {
  ...
  expect_true(nrow(d$data$return) != 0)
  d$steps <- TRUE
  expect_equal(names(d$return()), c("raw", "clean", "return", "processed"))
})
...
test_that("DataClass returns a summary", {
  ...
  expect_equal(
    names(sum),
    c(
      "country", "class", "level_1_region", "level_2_region",
      "get_data_function", "data_url", "source_data_cols"
    )
  )
