...
get_input_data_for_get_regional_covid_data_tests_with_level_2_regions <- function() {
  data <- get_input_data_for_get_regional_covid_data_tests_only_level_1_regions()
...
get_expected_data_for_get_regional_covid_data_tests_with_level_2_regions <- function() {
  data <- get_expected_data_for_get_regional_covid_data_tests_only_level_1_regions()
  data <- data[, -3]
...
get_expected_totals_data_for_get_regional_covid_data_tests_with_level_2_regions <- function() {
  data <- get_expected_totals_data_for_get_regional_covid_data_tests_only_level_1_regions()
...
get_expected_data_for_covid19R_converter_test <- function() {
    dates <- c("2020-01-31", "2020-02-01", "2020-02-02", "2020-02-03")
    date <- sort(rep(dates, 24))
    location <- rep(c(rep("Northland", 8), rep("Eastland", 8), rep("Wisconsin", 8)), 4)
    location_code <- rep(c(rep("NO", 8), rep("EA", 8), rep("WI", 8)), 4)
    exp_data_start <- cbind(date, location, location_code)

    # poke holes due to removal of hosp_total and tests_new
    first <- c(8, 9)
    removed_data_idx <- c()
    for (i in 0:11) {
      removed_data_idx <- c(removed_data_idx, first + (i*10))
    }

    set.seed(181)
    value <- floor(runif(120, 0, 100))[-removed_data_idx]
    set.seed(Sys.time())
