...
level_1 <- data.frame(level_1_region_code, level_1_region) %>%
  mutate(level_1_region = stri_trans_general(level_1_region, "latin-ascii"),
...
level_1 <- level_1 %>%
  mutate(level_1_region = ifelse(level_1_region %in% names(replacements),
...
data_l1 <- data_l1$download() %>%
  ...
  distinct()
inegi_l1 <- data_l1 %>%
  mutate(level_1_region = stri_trans_totitle(level_1_region),
